...
class QdrantCache:
    def __init__(self):
        ...
        if QDRANT_AVAILABLE and settings.SEMANTIC_CACHE_BACKEND == "qdrant":
            try:
                self.client = RealQdrantClient(host=settings.QDRANT_HOST, port=settings.QDRANT_PORT, timeout=2.0)
    ...
    def delete(self, entry_id: str):
        ...
        try:
            self.client.delete(
                collection_name=settings.QDRANT_COLLECTION, points_selector=qmodels.PointIdsList(points=[entry_id])
            )
        except Exception as e:
