...
def get_all_cached_entries() -> dict:
    ...
    cursor.execute(
        "SELECT key, repo, branch, created_at, tokens_input + tokens_output as total_tokens, file_hashes FROM exact_cache ORDER BY created_at DESC LIMIT 50"
    )
    exact = []
    ...
    cursor.execute(
        "SELECT id, repo, branch, prompt, created_at, tokens_input + tokens_output as total_tokens, file_hashes FROM semantic_cache ORDER BY created_at DESC LIMIT 50"
    )
    semantic = []
    for r in cursor.fetchall():
        semantic.append(
