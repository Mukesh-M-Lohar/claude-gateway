...
async def set_semantic_cache(repo: str, branch: str, commit_hash: str, prompt: str, normalized_prompt: str, response: str, tokens_input: int, tokens_output: int, file_hashes: dict):
    # 1. Generate prompt embedding
    try:
        embedding = await get_embedding(normalized_prompt)
    except Exception as e:
    ...
    if settings.SEMANTIC_CACHE_BACKEND == "qdrant" and qdrant_cache.is_connected():
        qdrant_cache.save(repo, branch, commit_hash, prompt, normalized_prompt, response, tokens_input, tokens_output, embedding, file_hashes)
    else:
